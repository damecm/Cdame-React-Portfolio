...
export default class PortForm extends Component {
  constructor (props) {
    ...
    this.state= {
      ...
      logo: "",
      editMode: false,
      apiUrl: "https://dameframe.devcamp.space/portfolio/portfolio_items",
      apiAction: "post"
    };
  ...
  componentDidUpdate() {
    if (Object.keys(this.props.portfolioToEdit).length > 0) {
      ...
      this.setState({
        ...
        url: url || "",
        editMode: true,
        apiUrl: `https://dameframe.devcamp.space/portfolio/portfolio_items/${id}`,
        apiAction: "patch",
  ...
  handleSubmit(event) {
    //rather then using ".then,.catch"
    //"key: value" setup below
    axios({
      ...
      .then(response => {
        if (this.state.editMode) {
          this.props.handleEditFormSubm();
        } else {
          this.props.handleNewFormSubm(response.data.portfolio_item);
        }
